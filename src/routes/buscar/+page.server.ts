...
export const load: PageServerLoad = async ({ url }) => {
	...
	let areas: BusquedaAreaRow[] = [];
	let totalResultados = 0;

	try {
		pias = await BusquedaDAO.getPIAs(query);
		proyectos = await BusquedaDAO.getProyectos(query);
		areas = await BusquedaDAO.getAreas(query);
		totalResultados = pias.length + proyectos.length + areas.length;
		return {
			pias, totalResultados, query, proyectos, areas
		}
